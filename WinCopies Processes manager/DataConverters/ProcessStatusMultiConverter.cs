...
namespace WinCopiesProcessesManager
{
    public class ProcessStatusMultiConverter : MultiConverterBase
    {
        public override object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if ((string)parameter == "ProgressStatus")
...

            {
                ...
                if ((WinCopies.IO.Size)values[1] > 0)

                    a = ((WinCopies.IO.Size)values[0] / (WinCopies.IO.Size)values[1]).Value; 

                return (WinCopies.IO.Size)values[1] > 0 ? ((WinCopies.IO.Size)values[0] / (WinCopies.IO.Size)values[1] * 100).Value : 0d;

            }

            else return (string)parameter == "DisplayProgressStatus"
                ? (bool)values[0]
                    ? !(bool)values[1]    
